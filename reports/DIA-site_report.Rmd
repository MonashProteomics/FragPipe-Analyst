...
The raw DIA data files were analyzed using workflow in FragPipe to obtain site-level quantification values.

Uploaded site-level report was converted to log2 scale, samples were grouped by conditions and missing values were imputed using `r params$imputation` method. Feature-wise linear models combined with empirical Bayes statistics were used for the differential expression analyses. The _limma_ package from R Bioconductor was used to generate a list of differentially expressed features for each pair-wise comparison. A cutoff of the _adjusted p-value_ of `r params$alpha` (`r params$fdr_correction` method) along with a |log2 fold change| of `r params$lfc` has been applied to determine differentially expressed features in each pairwise comparison.
...
### Result output contains `r nrow(params$data)` features of which _`r nrow(params$dep())`_ features were reproducibly quantified.

### `r params$num_signif` features differ significantly between samples.
...
Features quantified per sample (after pre-processing).
...
Feature overlap in all samples.
...
Feature distribution before and after imputation. The plot showing the effect of imputation on features' expression distribution.
...
### Heatmap
  A plot representing an overview of expression of all significant (differentially expressed) features (rows) in all samples (columns).
