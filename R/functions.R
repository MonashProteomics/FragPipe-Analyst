...
get_exp_design <- function(df){
  if(length(grep("^LFQ.", colnames(df))) !=0){
    intensity_cols <- grep("LFQ.intensity[.]", colnames(df))
    intensity_names <- colnames(df[,intensity_cols]) %>% 
      gsub("LFQ.intensity[.]", "", .) %>% unique() 
  } 
  else {
    # intensity_cols <- grep("Total.Intensity", colnames(df))
    # intensity_names <- colnames(df[,intensity_cols]) %>% 
    #   gsub(".Total.Intensity", "", .)  %>% 
    #   gsub(".MaxLFQ", "", .) %>% unique() 
    
    all_intentisy_cols <- grep(".Intensity", colnames(df))
    remove_cols <- grep("Unique.Intensity|Total.Intensity",colnames(df))
    intensity_cols <- setdiff(all_intentisy_cols, remove_cols)
